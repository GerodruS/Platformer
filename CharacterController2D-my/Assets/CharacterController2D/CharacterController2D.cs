...
namespace Prime31
{
    ...
    public class CharacterController2D : MonoBehaviour
    {
        ...
        private void primeRaycastOrigins()
        {
            ...
            _raycastOrigins.topLeft = transform.localPosition +
                new Vector3(boxCollider.offset.x - boxCollider.size.x * 1.0f,
                            boxCollider.offset.y + boxCollider.size.y * 1.0f,
                            0.0f);
            _raycastOrigins.bottomRight = transform.localPosition +
                new Vector3(boxCollider.offset.x + boxCollider.size.x * 1.0f,
                            boxCollider.offset.y - boxCollider.size.y * 1.0f,
                            0.0f);
            _raycastOrigins.bottomLeft = transform.localPosition +
                new Vector3(boxCollider.offset.x - boxCollider.size.x * 1.0f,
                            boxCollider.offset.y - boxCollider.size.y * 1.0f,
                            0.0f);
        }
        ...
        private void moveHorizontally(ref Vector3 deltaMovement)
        {
            ...
            var rayDirection = isGoingRight ? transform.parent.right : -transform.parent.right;
            ...
            for (var i = 0; i < totalHorizontalRays; i++)
            {
                ...
                var localRay = ray;
                ray = transform.parent.TransformPoint(ray);
                ...
                if (_raycastHit)
                {
                    ...
                    if (i == 0 && handleHorizontalSlope(ref deltaMovement, Vector2.Angle(_raycastHit.normal, transform.parent.up)))
                    {
                    ...
                    deltaMovement.x = transform.parent.InverseTransformPoint(_raycastHit.point).x - localRay.x;
            ...
            if (angle < slopeLimit)
            {
                ...
                if (deltaMovement.y < jumpingThreshold)
                {
                    ...
                    if (collisionState.wasGroundedLastFrame)
                        raycastHit = Physics2D.Raycast(ray, transform.parent.TransformDirection(deltaMovement.normalized), deltaMovement.magnitude, platformMask);
                    else
                        raycastHit = Physics2D.Raycast(ray, transform.parent.TransformDirection(deltaMovement.normalized), deltaMovement.magnitude, platformMask & ~oneWayPlatformMask);
                    if (raycastHit)
                    {
                        // we crossed an edge when using Pythagoras calculation, so we set the actual delta movement to the ray hit location
                        deltaMovement = transform.parent.InverseTransformPoint(raycastHit.point) - localRay;
                        if (isGoingRight)
                            deltaMovement.x -= _skinWidth;
                        else
                            deltaMovement.x += _skinWidth;
        ...
        private void moveVertically(ref Vector3 deltaMovement)
        {
            ...
            var rayDirection = isGoingUp ? transform.parent.up : -transform.parent.up;
            ...
            for (var i = 0; i < totalVerticalRays; i++)
            {
                ...
                DrawRay(ray, rayDirection * rayDistance * 2.0f, Color.green);
                _raycastHit = Physics2D.Raycast(ray, rayDirection, rayDistance, mask);
                if (_raycastHit)
                {
                    // set our new deltaMovement and recalculate the rayDistance taking it into account
                    //Debug.Break();
                    deltaMovement.y = transform.parent.InverseTransformPoint(_raycastHit.point).y - localRay.y;
                    //continue;
        ...
        private void handleVerticalSlope(ref Vector3 deltaMovement)
        {
            ...
            var rayDirection = -transform.parent.up;
            ...
            var localSlopeRay = slopeRay;
            slopeRay = transform.parent.TransformPoint(slopeRay);
            DrawRay(slopeRay, rayDirection * slopeCheckRayDistance, Color.yellow);
            _raycastHit = Physics2D.Raycast(slopeRay, rayDirection, slopeCheckRayDistance, platformMask);
            if (_raycastHit)
            {
                // bail out if we have no slope
                var angle = Vector2.Angle(_raycastHit.normal, transform.parent.up);
                if (angle == 0)
                    return;

                // we are moving down the slope if our normal and movement direction are in the same x direction
                var isMovingDownSlope = Mathf.Sign(transform.parent.InverseTransformDirection(_raycastHit.normal).x) == Mathf.Sign(deltaMovement.x);
                if (isMovingDownSlope)
                {
                    // going down we want to speed up in most cases so the slopeSpeedMultiplier curve should be > 1 for negative angles
                    var slopeModifier = slopeSpeedMultiplier.Evaluate(-angle);
                    // we add the extra downward movement here to ensure we "stick" to the surface below
                    deltaMovement.y += transform.parent.InverseTransformPoint(_raycastHit.point).y - localSlopeRay.y - skinWidth;
                    deltaMovement.x *= slopeModifier;
